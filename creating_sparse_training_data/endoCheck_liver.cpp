...
const int CROPWIDTH = 480;
const int CROPHEIGHT = 384;


EndoCheck::EndoCheck(bool computeHRM, char* path, char* out_path) :
...
void EndoCheck::computeSurf(Mat img_object, Mat img_scene)
{
    ...
    for (int i = 0; i < descriptors_object.rows; i++)
    {
        ...
        if ( matches[i].distance < 5 * min_dist && abs(y1 - y2) < 3  && x1 - x2 < 50 && x1 - x2 > 0)
        {
            ...
            disparities[(int)x1+(int)y1*CROPWIDTH] = x1 - x2;
            disparities[1+(int)x1+(int)y1*CROPWIDTH] = x1 - x2;
            disparities[(int)x1+(1+(int)y1)*CROPWIDTH] = x1 - x2;
            disparities[1+(int)x1+(1+(int)y1)*CROPWIDTH] = x1 - x2;
...
bool EndoCheck::Run()
{
    ...
    sprintf(left, "%sscene_left0000.bmp", m_path);
    sprintf(right, "%sscene_right0000.bmp", m_path);
    ...
    disparities = new double[CROPWIDTH*CROPHEIGHT];
    ...
    maskLeft = imread( "/home/staff/reichard/workspace/depthlearning/mask_left_liver.png", CV_LOAD_IMAGE_GRAYSCALE );
    maskRight = imread( "/home/staff/reichard/workspace/depthlearning/mask_right_liver.png", CV_LOAD_IMAGE_GRAYSCALE );
	...
	while (this->isVisible())
	{
		...
		if (next)
		{
            ...
            cv::Rect myROI(95, 102, 480, 384);

            grey_left = grey_left(myROI);
            grey_right = grey_right(myROI);
            ...
            out_left = out_left(myROI);
            out_right = out_right(myROI);
            ...
            {
                char file[200];
                sprintf(file, "%s/left_depth_%i.disp", m_out_path, frameNumber);
                ...
                sprintf(file, "%s/left_normal_%i.png", m_out_path, frameNumber);
                //inImages[0]->SaveToFile(file);
                imwrite(file, out_left);
                sprintf(file, "%s/right_normal_%i.png", m_out_path, frameNumber);
                //inImages[0]->SaveToFile(file);
...
int main(int argc, char** argv)
{
    ...
    if(argc == 3)
        window = new EndoCheck(true, argv[1],argv[2]);
	else
		return EXIT_FAILURE;
